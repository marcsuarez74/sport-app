...
| `.recette-card` / `.recette-top` | carte compacte repliée, toujours visible sous la ligne du repas (RecetteCard, fidélité Figma) | `--surface` + bordure + radius 16, padding 10 ; `.recette-thumb` 110×76 r12 (ou `.recette-fallback` gradient orange/lime + 🍳) + `.recette-main` : nom 13px line-clamp 3 + bouton lime `.recette-toggle` min 48px « Voir la recette ⌄ / Réduire ⌃ » — état local par carte (multi-ouvertes possibles) |
| `.recette-badges` | rangée badges sous le bloc image/nom | flex-wrap ; `.recette-badge-cat` lime (label du repas) + `.recette-badge-info` `--surface-2` (⏱ temps tronqué avant « · ») + score inline aligné à droite : `.stat-label` + valeur + `.score-bar` 97px (10 segments, `.score-seg.on` = accent) |
| `.recette-nutri` | footer nutrition | barre `--surface-2` radius 8, cellules conditionnelles 🔥 kcal / 🌾 C / 💪 P / 💧 F (11px muted, nowrap) séparées par traits verticaux `--border` |
| `.recette-detail` + `.recette-*` | détail déplié (séparateur pointillé) | pour, `.recette-bchip` cliquable (état `.on` = bordure accent), `.recette-etapes`, `.recette-mel` / `.recette-bat` pastilles lime/orange via `color-mix` + `::before` émoji |
